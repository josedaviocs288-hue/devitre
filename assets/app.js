(() => {
  ...
  const storeBy=id=>stores.find(s=>s.id===Number(id));
  ...
  function setLoginType(type){
    ...
    const acc=accounts[type];document.getElementById('loginEmail').value=acc.email;document.getElementById('loginPassword').value=acc.password;
  }
  function login(){
    const email=document.getElementById('loginEmail').value.trim().toLowerCase();const password=document.getElementById('loginPassword').value;const acc=accounts[loginType];
    if(email!==acc.email||password!==acc.password){toast('Use a conta de demonstração exibida na tela.');return;}
  ...
  function showApp(){
    ...
    document.getElementById('topAvatar').textContent=session.role==='seller'?'CB':initials(state.profile.name);
    document.getElementById('modeBar').innerHTML=session.role==='seller'?'<b>Modo loja:</b> Charlotts Boutique · gerencie vitrine, reservas, mensagens e estoque.':'<b>Modo cliente:</b> encontre peças locais, converse com lojas, reserve e retire presencialmente.';
  ...
  setLoginType('client');setTheme(state.theme||'light');updateBadges();
