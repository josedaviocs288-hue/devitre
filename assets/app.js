(() => {
  ...
  const images = [
    ...
    'https://images.unsplash.com/photo-1542280756-74b2f55e73e1?auto=format&fit=crop&w=900&q=82',
    'https://images.unsplash.com/photo-1551028719-00167b16eac5?auto=format&fit=crop&w=900&q=82',
    'https://images.unsplash.com/photo-1593025251662-7f9a1cb1c75c?auto=format&fit=crop&w=900&q=82',
    ...
    'https://images.unsplash.com/photo-1594892725117-4a7a08b2c0b4?auto=format&fit=crop&w=900&q=82',
    'https://images.unsplash.com/photo-1612336307429-8a898d10e223?auto=format&fit=crop&w=900&q=82',
    'https://images.unsplash.com/photo-1595828204607-0d09afb58a71?auto=format&fit=crop&w=900&q=82',
  ...
  const seedProducts = [
    ...
    {id:'p2',store:1,name:'Casaco Longo de Lã',category:'Casaco',price:189.90,image:images[1],sizes:['P','M','G'],color:'Caramelo',stock:5,material:'Lã',featured:true},
    {id:'p3',store:1,name:'Vestido Coluna Clássico',category:'Vestido',price:169.90,image:images[2],sizes:['P','M','G'],color:'Preto',stock:2,material:'Crepe estruturado',new:true},
    {id:'p4',store:2,name:'Conjunto Alfaiataria Urbano',category:'Conjunto',price:199.90,image:images[3],sizes:['P','M','G'],color:'Neutro',stock:3,material:'Alfaiataria',featured:true},
    {id:'p5',store:2,name:'Look Clássico Essencial',category:'Conjunto',price:189.90,image:images[4],sizes:['P','M','G'],color:'Neutro',stock:4,material:'Composição mista'},
    {id:'p6',store:3,name:'Conjunto Tricô & Wide Leg',category:'Conjunto',price:184.90,image:images[5],sizes:['P','M','G'],color:'Neutro',stock:5,material:'Tricô e alfaiataria'},
    {id:'p7',store:3,name:'Saia Midi Evasê',category:'Saia',price:119.90,image:images[6],sizes:['P','M','G'],color:'Preto',stock:3,material:'Tecido leve'},
    {id:'p8',store:4,name:'Jaqueta Jeans Estruturada',category:'Casaco',price:179.90,image:images[7],sizes:['P','M','G'],color:'Jeans',stock:4,material:'Denim',featured:true},
    {id:'p9',store:4,name:'Blusa Manga Bufante',category:'Blusa',price:94.90,image:images[8],sizes:['P','M','G'],color:'Claro',stock:6,material:'Tecido leve'},
    {id:'p10',store:5,name:'Trench Coat em Couro',category:'Casaco',price:229.90,image:images[9],sizes:['P','M','G'],color:'Marrom',stock:3,material:'Couro',new:true},
    {id:'p11',store:5,name:'Calça Jeans Cropped',category:'Calça',price:139.90,image:images[10],sizes:['36','38','40','42'],color:'Jeans',stock:4,material:'Denim'},
    {id:'p12',store:6,name:'Vestido Floral de Verão',category:'Vestido',price:154.90,image:images[11],sizes:['P','M','G'],color:'Floral',stock:4,material:'Tecido fluido'},
    {id:'p13',store:6,name:'Vestido Cintura Império',category:'Vestido',price:169.90,image:images[12],sizes:['P','M','G'],color:'Neutro',stock:7,material:'Tecido leve'},
    {id:'p14',store:7,name:'Vestido Elegance',category:'Vestido',price:189.90,image:images[13],sizes:['P','M','G'],color:'Preto',stock:3,material:'Crepe'},
    {id:'p15',store:7,name:'Vestido Midi Bodycon',category:'Vestido',price:159.90,image:images[14],sizes:['P','M','G'],color:'Vermelho',stock:6,material:'Malha estruturada'},
    {id:'p16',store:8,name:'Blusa Floral Delicada',category:'Blusa',price:79.90,image:images[15],sizes:['P','M','G'],color:'Floral',stock:7,material:'Viscose',new:true},
    {id:'p17',store:8,name:'Vestido Feminino Clássico',category:'Vestido',price:169.90,image:images[16],sizes:['P','M','G'],color:'Neutro',stock:4,material:'Tecido encorpado'},
    {id:'p18',store:9,name:'Blusa Casual Clássica',category:'Blusa',price:79.90,image:images[17],sizes:['P','M','G'],color:'Neutro',stock:5,material:'Algodão'},
    {id:'p19',store:9,name:'Calça Wide Leg',category:'Calça',price:149.90,image:images[18],sizes:['36','38','40','42'],color:'Preto',stock:3,material:'Alfaiataria',featured:true}
  ];

  const seededProductMap = Object.fromEntries(seedProducts.map(p=>[String(p.id),p]));
  function normalizeSeedCatalog(products){
    return (Array.isArray(products)?products:[]).map(p=>{
      const corrected=seededProductMap[String(p.id)];
      if(!corrected) return p;
      return {...p,...corrected,stock:Number.isFinite(Number(p.stock))?Number(p.stock):corrected.stock};
    });
  }
  ...
  const productReviews = {
    ...
    p8:[{name:'Ana',stars:4,text:'Jaqueta bonita, veste bem e tem ótimo acabamento.'}]
  };
  ...
  function loadState(){
    try{
      ...
      if(raw){const parsed=JSON.parse(raw);const merged={...freshState(),...parsed};merged.products=normalizeSeedCatalog(merged.products);return merged;}
      const legacy=localStorage.getItem(LEGACY_KEY);
      if(legacy){const migrated=migrateLegacy(JSON.parse(legacy));migrated.products=normalizeSeedCatalog(migrated.products);return migrated;}
    }catch{}
    return freshState();
  }
  state=loadState();
  ...
  function renderHome(){
    ...
    document.getElementById('page-home').innerHTML=`
      ...
      <section class="section"><div class="section-head"><div><h2>Comece pelo seu estilo</h2><div class="sub">Categorias rápidas para encontrar sua próxima peça.</div></div></div><div class="category-grid">${[['Vestido','✦'],['Blusa','◌'],['Calça','▱'],['Conjunto','◇'],['Casaco','⌁'],['Saia','◐'],['Tudo','↗']].map(([c,i])=>`<button class="category-card" data-category="${c==='Tudo'?'':c}"><span>${i}</span><div><b>${c}</b><br><small>${c==='Tudo'?'Ver catálogo completo':'Explorar seleção'}</small></div></button>`).join('')}</div></section>
